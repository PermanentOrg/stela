...
describe("processPendingInvites", () => {
	...
	test("should grant recursive access to the shared folder_link and its descendants", async () => {
		...
		expect(result.rows).toEqual([
			{ folder_linkid: "1", archiveid: "2", accessrole: "access.role.editor" },
			{ folder_linkid: "2", archiveid: "2", accessrole: "access.role.editor" },
		]);
	});
	...
	test("should use the most permissive access role when two invites name the same folder_link", async () => {
		await processPendingInvites("newaccount@permanent.org", "2", db);
		...
		const accessResult = await db.query<{ accessRole: string }>(
			'SELECT accessrole AS "accessRole" FROM access WHERE folder_linkid = 1',
		);
		expect(accessResult.rows).toHaveLength(1);
		expect(accessResult.rows[0]?.accessRole).toEqual("access.role.editor");
	...
	test("should accept an invite_share with an unrecognized access role without granting anything", async () => {
		...
		expect(processedInviteIds).toHaveLength(0);
	});
	...
	test("should throw if creating the share fails", async () => {
		mockSqlCall(
			...
			{
				folderLinkIds: ["1"],
				accessRoles: ["access.role.editor"],
				archiveId: "2",
			},
			{ reject: new Error("test error") },
		);
