...
FROM shared_subtree
WHERE NOT EXISTS (
  SELECT existing_access.accessid
  FROM access AS existing_access
  WHERE
    existing_access.folder_linkid = shared_subtree.folder_linkid
    AND existing_access.archiveid = shared_subtree.archiveid
);
