WITH lock_archive_part AS MATERIALIZED (
  SELECT PG_ADVISORY_XACT_LOCK(HASHTEXT('archive_nbr_archive_part')) AS acquired
),

next_archive_part AS (
  SELECT
    COALESCE(INCREMENT_BASE36(MAX(archive_nbr.archivepart)), '0000')
      AS archive_part
  FROM archive_nbr
  CROSS JOIN lock_archive_part
),
