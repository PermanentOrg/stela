data "kubernetes_deployment" "stela_staging" {
	metadata { name = "stela-staging" }
}

locals {
	current_image = { stela_staging_image = data.kubernetes_deployment.stela_staging.spec[0].template[0].spec[0].container[0].image }
