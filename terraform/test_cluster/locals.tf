locals {
  ...
  current_kubernetes_images = merge(
    ...
    try({ for container in data.kubernetes_resource.event_send_dev[0].object.spec.jobTemplate.spec.template.spec.containers : container.name => container.image }, {}),
    try({ for container in data.kubernetes_resource.event_send_staging[0].object.spec.jobTemplate.spec.template.spec.containers : container.name => container.image }, {}),
