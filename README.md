...
Update values as needed — each variable is documented with inline comments.
...
Environment variables are documented inline in [`.env.template`](.env.template). Copy it to `.env` and update values as needed.
